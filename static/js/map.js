    ...
    fetch('/get_route?check_traffic=true')
        ...
        .then(data => {
            if (data && data.has_traffic_update) {
                ...
                showTrafficUpdateNotification(data.traffic_update_reason || 'Route updated with current traffic conditions');
        ...
        .catch(error => {
            console.error('Error checking for traffic updates:', error);
            // Disable future traffic checks if we have persistent errors
...
function displayRoute(routeData) {
    ...
    if (segments && segments.length > 0) {
        ...
        for (const segment of segments) {
            ...
            // Sprawdzamy, czy mamy zakodowaną geometrię (polyline) czy zwykłą tablicę punktów
            if (segment.encoded_geometry) {
                // Mamy zakodowaną geometrię - użyjmy jej dla dokładnego śledzenia dróg
                try {
                    console.log("Dekodowanie polyline dla segmentu", segment.start_idx, "-", segment.end_idx);
                    // Dekodowanie polyline zwraca punkty w formacie [lat, lon], idealnym dla Leaflet
                    segmentPoints = polyline.decode(segment.encoded_geometry);
                    console.log(`Segment [${segment.start_idx}-${segment.end_idx}]: Zdekodowano ${segmentPoints.length} punktów`);
                } catch (e) {
                    console.error("Błąd dekodowania polyline:", e);
                    // Fallback - użyjemy standard geometry, czyli linii prostej
                    if (segment.geometry && Array.isArray(segment.geometry)) {
                        segmentPoints = segment.geometry.map(coord => [coord[1], coord[0]]);
                    } else {
                        const start = coordinates[segment.start_idx];
                        const end = coordinates[segment.end_idx];
                        segmentPoints = [[start[1], start[0]], [end[1], end[0]]];
                    }
                }
            } else if (segment.geometry) {
                // Brak zakodowanej geometrii, używamy standardowej geometrii
                if (typeof segment.geometry === 'string') {
                    // Standardowa geometria jako string (mało prawdopodobne, ale zabezpieczamy się)
                    try {
                        segmentPoints = polyline.decode(segment.geometry);
                    } catch (e) {
                        console.error("Błąd dekodowania geometry string:", e);
                        const start = coordinates[segment.start_idx];
                        const end = coordinates[segment.end_idx];
                        segmentPoints = [[start[1], start[0]], [end[1], end[0]]];
                    }
                } else if (Array.isArray(segment.geometry)) {
                    // Geometria jako tablica punktów [lon, lat] - konwertujemy na [lat, lon] dla Leaflet
                    segmentPoints = segment.geometry.map(coord => {
                        return [coord[1], coord[0]]; // Zamiana [lon, lat] na [lat, lon]
                    });
                }
            } else {
                console.error("Brak danych geometrycznych dla segmentu");
                
                // Fallback - użyj punktów startowy i końcowy jako linii prostej
                if (segment.start_idx !== undefined && segment.end_idx !== undefined && 
                    coordinates[segment.start_idx] && coordinates[segment.end_idx]) {
                    const start = coordinates[segment.start_idx];
                    const end = coordinates[segment.end_idx];
                    segmentPoints = [[start[1], start[0]], [end[1], end[0]]];
                } else {
                    return; // Brak wystarczających danych do narysowania segmentu
                }
            }
            
            // Jeśli mamy już punkty, możemy narysować linię
            if (segmentPoints.length > 0 && typeof segmentPoints[0] !== 'undefined') {
                console.log("Rysowanie segmentu z", segmentPoints.length, "punktami");
    ...
    // Add traffic lights to the map if available
    if (routeData.route_details && 
        routeData.route_details.traffic_light_analysis && 
        routeData.route_details.traffic_light_analysis.traffic_lights && 
        routeData.route_details.traffic_light_analysis.traffic_lights.length > 0) {
        
        const trafficLights = routeData.route_details.traffic_light_analysis.traffic_lights;
        
        trafficLights.forEach((light, index) => {
            if (light.coordinates && light.coordinates.length === 2) {
                // Create a custom traffic light icon
                const trafficLightIcon = L.divIcon({
                    className: 'traffic-light-marker',
                    html: `<div class="traffic-light-icon">
                ...
                // Add the traffic light marker to the map
                const marker = L.marker([light.coordinates[1], light.coordinates[0]], {
                    icon: trafficLightIcon,
                    zIndexOffset: 1000  // Make sure traffic lights appear above other markers
                }).addTo(map);
                
                // Add a popup with traffic light information
                let popupContent = `
                    <div class="traffic-light-popup">
                        <h6>Traffic Light #${index + 1}</h6>
                        <p>Cycle time: ${light.cycle_time} seconds</p>
                `;
                
                if (light.optimization && light.optimization.message) {
                    popupContent += `<p class="mb-1"><i class="fas fa-info-circle me-1"></i> ${light.optimization.message}</p>`;
                    
                    if (light.optimization.time_saved > 0) {
                        popupContent += `<p class="text-success mb-0">
                            <i class="fas fa-clock me-1"></i> Save ${Math.round(light.optimization.time_saved)} seconds
                        </p>`;
                    }
                }
...
function updateRouteSummary(routeData) {
    ...
    // Create traffic light information if available
    let trafficLightHtml = '';
    if (routeData.traffic_light_analysis && 
        routeData.traffic_light_analysis.traffic_lights && 
        routeData.traffic_light_analysis.traffic_lights.length > 0) {
        
        const lights = routeData.traffic_light_analysis.traffic_lights;
        const recommendations = routeData.traffic_light_analysis.recommendations || [];
        const totalTimeSaved = routeData.traffic_light_analysis.total_time_saved || 0;
        
        trafficLightHtml = `
            <div class="mt-3 card bg-dark text-light">
                <div class="card-header bg-primary">
                    <h6 class="mb-0"><i class="fas fa-traffic-light me-2"></i>Smart Traffic Light Analysis</h6>
                </div>
                <div class="card-body">
                    <p class="mb-2">
                        <i class="fas fa-info-circle me-1"></i> 
                        Wykryto ${lights.length} sygnalizacji świetlnych na trasie
                    </p>
        `;
        ...
        if (totalTimeSaved > 0) {
            trafficLightHtml += `
                <div class="alert alert-success py-2">
                    ...
                    Optymalizacja świateł może zaoszczędzić ${Math.round(totalTimeSaved)} sekund
                </div>
            `;
        }
        
        if (recommendations && recommendations.length > 0) {
            trafficLightHtml += `<div class="mt-2"><strong>Zalecenia:</strong>`;
            trafficLightHtml += `<ul class="mb-0 ps-3 mt-1">`;
            recommendations.forEach(rec => {
                trafficLightHtml += `<li>${rec}</li>`;
            });
            trafficLightHtml += `</ul></div>`;
        }
